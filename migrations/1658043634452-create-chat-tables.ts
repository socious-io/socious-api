...
export class createChatTables1658043634452 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`user_chats\` (\`id\` bigint UNSIGNED NOT NULL AUTO_INCREMENT, \`userId\` bigint UNSIGNED NOT NULL, \`muted\` tinyint NOT NULL DEFAULT false, \`deleted\` tinyint NOT NULL DEFAULT false, \`lastReadId\` int NULL, \`lastReadDT\` datetime NULL, \`allRead\` tinyint NOT NULL DEFAULT false, \`createdAt\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updatedAt\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`chatId\` bigint UNSIGNED NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`chat_messages\` (\`id\` bigint UNSIGNED NOT NULL AUTO_INCREMENT, \`userId\` bigint UNSIGNED NULL, \`text\` mediumtext NULL, \`createdAt\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updatedAt\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`media\` varchar(255) NULL, \`mediaType\` varchar(255) NULL, \`chatId\` bigint UNSIGNED NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
