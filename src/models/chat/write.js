...
export const create = async (identityId, body) => {
  ...
  try {
    ...
    `)
    const chat = rows[0]
    await body.participants.map(p => addParticipant(chat.id, p, identityId, MemberTypes.ADMIN))
    await Promise.all(
...
export const addParticipant = async (chatId, participantId, joinedById, type=MemberTypes.MEMBER) => {
  try {
  const {rows} = await app.db.query(sql`
...
export const newMessage = async (id, identityId, text, replyId=null) => {
  try {
  const {rows} = app.db.query(sql`
    INSERT INTO messages (identity_id, chat_id, text, reply_id)
    VALUES (${identityId}, ${id}, ${text}, ${replyId}) RETURNING id
  `)
  return rows[0]
  } catch(err) {
    throw new EntryError(err.message)
